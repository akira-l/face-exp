...
# 	trainset = datasets.ImageFolder(root = datapath + 'train',transform = training_transform)
	...
	trainset = ClassifyDataset("../face_a/face_image/train/", classifyTrainCsv, training_transform)
...
# 	testset = datasets.ImageFolder(root = datapath + 'test',transform = testing_transform)
	testset = ClassifyDataset("../face_a/face_image/train/", classifyValCsv, testing_transform)
...
# 		net = torchvision.models.resnet.resnet50(**kwargs)
	...
	classifyInFeature = net.fc5.in_features
	net.fc5 = nn.Linear(classifyInFeature, kwargs["num_classes"])

	face_loss = cosface_loss.MarginCosineProduct(2874, 2874).cuda()
...
# 	optimizer = optim.SGD(net.parameters(), lr=args.learning_rate, betas=(0.9, 0.999), momentum=0.9,  eps=1e-08, weight_decay=0)
	optimizer = optim.SGD([{'params':net.parameters()} , {'params':face_loss.parameters()}], lr=args.learning_rate, momentum=0.9, weight_decay=1e-4)
	#optimizer = optim.SGD(net.parameters(), lr=args.learning_rate, momentum=0.9, weight_decay=1e-4)
	# Training
	print('==> Init trainer..')
	from trainer import Trainer
	train = Trainer(net, face_loss, trainloader, testloader, optimizer, start_epoch=start_epoch,
